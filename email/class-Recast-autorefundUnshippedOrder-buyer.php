...
class Recast_Email_autorefundUnshippedOrder_buyer extends WC_Email {
    ...
    // Email slug we can use to filter other data.
		...
		$this->title       = __( 'Recast - autorefundUnshippedOrder() - buyer', 'castback-email' );
    ...
    // For admin area to let the user know we are sending this email to customers.
		...
		$this->subject     = sprintf( _x( 'Order #{order_number}: Autorefund Unshipped Order', 'An email sent to the buyer when an offer has been paid, but not shipped.', 'castback-email' ), 'Recast' );
	...
	function trigger( $user_email, $order_id ) {
		...
		$this->subject     = sprintf( _x( 'Order #'.$order_id.': Autorefund Unshipped Order', 'An email sent to the buyer when an offer has been paid, but not shipped.', 'castback-email' ), 'Recast' );
